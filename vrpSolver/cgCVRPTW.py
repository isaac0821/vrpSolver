    ...
    if (type(edges) is not dict):
        if (edges == 'Complete_Euclidean'):
            ...
            for i in customerID:
                for j in customerID:
                    if (i != j):
                        edges[i, j] = distEuclidean2D(nodes[i]['loc'], nodes[j]['loc'])
            for i in customerID:
                edges[depotID, i] = distEuclidean2D(nodes[depotID]['loc'], nodes[i]['loc'])
                edges[i, depotID] = distEuclidean2D(nodes[i]['loc'], nodes[depotID]['loc'])
        else:
