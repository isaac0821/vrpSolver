...
A `line` data type is a list of two :ref:`pt` s, the format is [:ref:`pt`, :ref:`pt`]. It can define lines, line segments, or rays. It will be considered differently when it is used in different functions. For example, for
	>>> s = [(0, 0), (5, 5)]
s is considered as a line in the function :func:`~geoVeRoPy.geometry.isPtOnLine()`
	>>> res = geoVeRoPy.isPtOnLine(pt = (1, 1), line = s)
s is considered as a line segment in the function :func:`~geoVeRoPy.geometry.isPtOnSeg()`
	>>> res = geoVeRoPy.isPtOnSeg(pt = (1, 1), seg = s)
s is considered as a ray in the function :func:`~geoVeRoPy.geometry.isPtOnRay()`
